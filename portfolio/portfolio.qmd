...
Piece randomness is handled entirely in firmware through a small random module. At startup, the STM32L432KC initializes this module once, which sets up an internal state variable for a lightweight pseudo-random number generator. From that point on, the main game loop (and/or timer/interrupt code) periodically updates this state and writes a 3-bit value into the global variable g_random3.
...
formal certification methodologies and hardware implementations of the
